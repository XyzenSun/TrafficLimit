...
get_traffic_usage() {
    ...
    log_message "流量统计周期开始于: $start_date" >&2
    ...
    # 获取vnstat输出
    local vnstat_output
    if ! vnstat_output=$(vnstat -i "$MAIN_INTERFACE" --begin "$start_date" --oneline b 2>/dev/null); then
        handle_error "无法获取网络接口 $MAIN_INTERFACE 的流量数据，请检查vnstat配置"
    fi
    
    # 验证vnstat输出格式
    if [ -z "$vnstat_output" ]; then
        handle_error "vnstat返回空数据"
    fi
    
    local field_count
    field_count=$(echo "$vnstat_output" | tr ';' '\n' | wc -l)
    if [ "$field_count" -lt 11 ]; then
        handle_error "vnstat输出格式异常，字段数量不足: $field_count"
    fi
...
check_and_limit_traffic() {
    local current_usage
    if ! current_usage=$(get_traffic_usage 2>&1); then
        handle_error "无法获取当前流量使用情况"
    fi
